...
class DAttentionBaseline(nn.Module):
    ...
    def __init__(
        ...
        use_pe:bool=True, dwc_pe:bool=False,fixed_pe:bool=False, log_cpb:bool=False
    ):
        """
        Parameters
            q_size,kv_size: (B,c,h,w)
            n_heads, n_head_channels, n_groups: 多头注意力参数
            attn_drop, proj_drop: ...
            stride,k_size: 计算offset的卷积层参数
            no_off: 是否不使用偏移量
            use_pe: 是否使用位置编码
            dwc_pe:是否使用深度可分离卷积进行位置编码
            fixed_pe:是否使用固定位置编码
            log_cpb: 是否使用对数中心位置偏差作为一种位置编码方式
        """
    ...
    def forward(self, q,kv):
        ...
        if self.no_off:
            x_sampled = F.avg_pool2d(kv, kernel_size=self.stride, stride=self.stride)
...
if __name__=="__main__":
    """
    q_size,kv_size 参考实际
    n_heads, n_head_channels, n_group,按需求设
    attn_drop,proj_dropm offset_range_factor, 参考了DAT源码的config
    use_pe,dwc_pe,no_off, fixed_pe, 参考了源码config
