...
def MVTecDatasetDiffusion(c,is_train):
    subclass_path= os.path.join(c.data_path, c.class_name)
    sub_class=c.class_name
    img_size=c.input_size
    if is_train:
        args={"anomaly_source_path":c.dtd_path,
              "mvtec_root_path":c.data_path}
        return MVTecTrainDataset(subclass_path,sub_class,img_size=img_size,args=args)
    return MVTecTestDataset(subclass_path,sub_class,img_size=img_size)
