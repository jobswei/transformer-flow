...
def model_forward(c, extractor, conv_neck,transformer_flow, fusion_flow, image):
    ...
    y=conv_neck(h_list)
    z_list,jac_lis=transformer_flow(y)
    z_list, fuse_jac = fusion_flow(z_list)
    jac = fuse_jac + sum(jac_lis)

    return z_list, jac

def train_meta_epoch(c, epoch, loader, extractor, conv_neck,transformer_flow, fusion_flow, params, optimizer, warmup_scheduler, decay_scheduler, scaler=None):
    conv_neck=conv_neck.train()
    transformer_flow=transformer_flow.train()
    ...
    for sub_epoch in range(c.sub_epochs):
        ...
        for idx, (image, _, _) in tqdm.tqdm(enumerate(loader),total=len(loader)):
            ...
            if scaler:
                with autocast():
                    z_list, jac = model_forward(c, extractor, conv_neck,transformer_flow, fusion_flow, image)
                    loss = 0.
                    for z in z_list:
...
def inference_meta_epoch(c, epoch, loader, extractor, conv_neck,transformer_flow, fusion_flow):
    conv_neck=conv_neck.eval()
    transformer_flow=transformer_flow.eval()
...
def train(c):
    ...
    conv_neck,transformer_flow,fusion_flow=build_transformer_flow_model(c,output_channels,dims)
    conv_neck=conv_neck.to(c.device)
    transformer_flow=transformer_flow.to(c.device)
    ...
    params = list(fusion_flow.parameters())+list(transformer_flow.parameters())+list(conv_neck.parameters())
    ...
    start_epoch = 0
    if c.mode == 'test':
        start_epoch = load_weights(conv_neck,transformer_flow, fusion_flow, c.eval_ckpt)
        epoch = start_epoch + 1
    ...
    if c.resume:
        last_epoch = load_weights(conv_neck,transformer_flow, fusion_flow, os.path.join(c.ckpt_dir, 'last.pt'), optimizer)
    ...
    for epoch in range(start_epoch, c.meta_epochs):
        print()
        train_meta_epoch(c, epoch, train_loader, extractor, conv_neck,transformer_flow, fusion_flow, params, optimizer, warmup_scheduler, decay_scheduler, scaler if c.amp_enable else None)
        ...
        save_weights(epoch, conv_neck,transformer_flow, fusion_flow, 'last', c.ckpt_dir, optimizer)
        if best_det_auroc and c.mode == 'train':
            save_weights(epoch, conv_neck,transformer_flow, fusion_flow, 'best_det', c.ckpt_dir)
        if best_loc_auroc and c.mode == 'train':
            save_weights(epoch, conv_neck,transformer_flow, fusion_flow, 'best_loc_auroc', c.ckpt_dir)
        if best_loc_pro and c.mode == 'train':
            save_weights(epoch, conv_neck,transformer_flow, fusion_flow, 'best_loc_pro', c.ckpt_dir)
